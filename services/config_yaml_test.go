...
`
	...
	config, err := NewConfigFromFileWithEnv(configPath, mockEnv)
...
func TestNewConfigFromFile_NonExistent(t *testing.T) {
	config, err := NewConfigFromFile("/non/existent/config.yaml")
	assert.Error(t, err)
	assert.Nil(t, config)
	assert.Contains(t, err.Error(), "failed to load config file")
}

func TestNewConfigFromFileWithEnv_NonExistentExplicitPath(t *testing.T) {
	...
	config, err := NewConfigFromFileWithEnv("/explicit/non/existent/config.yaml", mockEnv)
	assert.Error(t, err)
	assert.Nil(t, config)
	assert.Contains(t, err.Error(), "failed to load config file")
...
func TestNewConfigFromFileWithEnv_EmptyConfigPath(t *testing.T) {
	...
	config, err := NewConfigFromFileWithEnv("", mockEnv)
	assert.NoError(t, err)
	assert.NotNil(t, config)
...
func TestNewConfigFromFileWithEnv_EnvOverrides(t *testing.T) {
	tempDir := t.TempDir()
	configPath := filepath.Join(tempDir, "test-config.yaml")
