...
import (
	...
	cmdproject "github.com/oar-cd/oar/cmd/project"
...
func NewCmdRoot() *cobra.Command {
	...
	It handles cloning, updates, and deployments with full state tracking.`,
		...
		PersistentPreRun: func(cmd *cobra.Command, args []string) {
			...
			var err error
			cfg, err = config.NewConfig(config.ConfigPath, config.WithCLIDefaults())
			if err != nil {
			...
			if err := app.InitializeWithConfig(cfg); err != nil {
	...
	cmd.AddCommand(cmdproject.NewCmdProject())
